...
use crate::filters::{with_db_connection, with_json_body, with_jwt_auth};
use crate::models::json::FormSchema;
use crate::rejections::ApiReject;
use crate::replies::ApiReply;

pub fn register(config: Config, db_pool: DbPool) -> impl Filter<Extract=impl Reply, Error=warp::Rejection> + Clone {
    ...
    let create_form = warp::path!("forms")
        .and(warp::post())
        .and(with_jwt_auth(config.clone()))
        .and(with_json_body::<FormSchema>(None))
...
async fn create_form(
    ...
    if let Err(error) = schema.validate() {
        return Err(warp::reject::custom(
            ApiReject::bad_request("Invalid form schema", Some(vec![error]))
        ));
    }
...
async fn update_form(_form_id: String, user_id: i64, schema: FormSchema) -> Result<impl Reply, warp::Rejection> {
    Ok("update form")
}
